...
var generateStyle = function generateStyle(isPageBackground, styleIn) {
  var output = _extends({
    overflow: 'clip',
    position: 'relative'
  }, styleIn);

  if (isPageBackground) output = _extends({}, output, {
    width: '100vw',
    height: '100vh',
    position: 'fixed',
    top: 0,
...
var StarSky = function StarSky(props) {
  var sketchRef = React.useRef();

  var _useState = React.useState(generateStyle(props.isPageBackground, props.style)),
      sketchStyle = _useState[0];
  ...
  var sketch = function sketch(p) {
    ...
    p.setup = function () {
      var cnv = p.createCanvas(sketchRef.current.offsetWidth, sketchRef.current.offsetHeight, p.P2D);
      ...
      _proto.shuffle = function shuffle() {
        if (this.shufflePerSecond > 0) {
          if (p.millis() > this.shuffleTimer + 1000 / this.shufflePerSecond) {
            var starIndex = Math.floor(Math.random() * this.count);
